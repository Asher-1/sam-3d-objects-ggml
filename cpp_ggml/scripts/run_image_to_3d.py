#!/usr/bin/env python3
"""Run raw image + mask -> ggml 3D entity -> official-render comparison.

MoGe and image preprocessing remain the official PyTorch implementation. All
conditioned generative stages and the Gaussian decoder are executed by ggml.
Reference noise can be replayed when the dumped occupancy shape matches the
backend output; otherwise the runtime uses its deterministic C++ PRNG.
"""
...
def backend_env(build_dir: Path, backend: str, dtype: str, threads: int,
                ...
                profile_jsonl: Path | None = None) -> dict[str, str]:
    env = os.environ.copy()
    ...
    env.pop("SAM3D_E2E_SKIP_COND", None)
    env.pop("SAM3D_E2E_STAGE", None)
    env.pop("SAM3D_E2E_REFERENCE_COORDS", None)
...
def runtime_provenance(executable: Path, models_dir: Path, dtype: str,
                       ...
                       slat_dtype: str | None,
                       gaussian_dtype: str | None) -> dict[str, object]:
...
def main() -> int:
    ...
    condition_env = backend_env(build_dir, args.backend, args.dtype, args.threads,
                                 ...
                                 args.flash_attn_kernel, profile_jsonl)
    condition_env["SAM3D_E2E_STAGE"] = "cond"
    condition_command = [
        str(executable), "e2e", "--model", str(args.models_dir.resolve()),
        str(conditions.resolve()),
        "--out", str(condition_prefix.resolve()), "--seed", str(args.seed),
    ...
    if not args.strict_reference_coords:
        ...
        ss_command = [
            str(executable), "e2e", "--model", str(args.models_dir.resolve()),
            str(conditions.resolve()),
            "--out", str(ss_prefix.resolve()), "--seed", str(args.seed),
            "--threads", str(args.threads),
        ]
    ...
    slat_command = [
        str(executable), "e2e", "--model", str(args.models_dir.resolve()),
        str(conditions.resolve()),
        "--out", str(ggml_ply.resolve()), "--seed", str(args.seed),
        "--threads", str(args.threads),
    ]
    if args.replay_noise:
    ...
    summary = {
        ...
        "ggml_flow_ms": ss_ms + slat_ms,
        "ggml_e2e_ms": condition_ms + ss_ms + slat_ms,
